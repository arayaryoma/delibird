...
const bannedChannels =
    process.env.BANNED_CHANNELS && process.env.BANNED_CHANNELS.split(",");

// Middleware to bail early if the channel is banned
export default function channelIsBanned(req, res, next) {
    ...
    if (channel && bannedChannels && bannedChannels.includes(channel)) {
        return res
            .status(403)
            .send("Channel has been disabled due to too many connections.");
    }
